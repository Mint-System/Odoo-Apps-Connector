...
class MrpProduction(models.Model):
    ...
    def action_confirm(self):
        for production in self:
            ...
            postprod_picking_type = self.env["stock.picking.type"].search([("kardex_picking_type", "=", "kardex_postprod")])

            error_message = []
            if not prod_picking_type:
                error_message.append(_("There is no picking type associated to kardex picking type = kardex_prod. Please correct this in the configuration."))
            if not postprod_picking_type:
                error_message.append(_("There is no picking type associated to kardex picking type = kardex_postprod. Please correct this in the configuration."))
