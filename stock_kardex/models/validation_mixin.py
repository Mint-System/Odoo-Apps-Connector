    ...
    def validate_function(self, kardex_picking_types):
        ...
        # check if picking types with kardex_picking_type = kardex_prod exist
            ...
            for kardex_picking_type in kardex_picking_types:
                picking_type = self.env["stock.picking.type"].search([("kardex_picking_type", "=", kardex_picking_type)])

                if not picking_type:
                    error_message.append(_(f"There is no picking type associated to kardex picking type = {kardex_picking_type}. Please correct this in the configuration."))
